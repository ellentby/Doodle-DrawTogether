...
<h2>解説の流れ</h2>
 <ul>
 		 <li><b>機能説明</b><br/>
  		機能に対応するmBaaSの機能（「会員管理」、「データストア」、「ファイルストア」）や、アプリ機能を説明します。</li>
		  <li><b>主な手順</b><br/>
		  機能を作成する手順や、キーコードとそれに関するUnityの操作を紹介します。
		  </li>
		  <li><b>動作確認</b><br/>
		  操作の結果をダッシュボードで確認します。
		  </li>
		  ...
		  <li><b>課題</b><br/>
		  このドキュメントは、アプリ全機能の説明はせず、代表的な機能のみ説明しています。説明できなかった機能は「課題」として実装方法を考えて見ましょう。
	  	　<br/>課題の答えは全てプロジェクトコードにあります。答えを探す方法は<a href="#discussionanswer">こちら</a>。
		  </li>
</ul>
...
<ul>
  <li><b>機能説明</b><br/>
  	<ul>
	<li>mBaaSについて<br/>
  	この機能を開発するには、mBaaSの「会員管理」機能を利用します。「会員管理」は、アプリ利用者に会員登録を意識させない形で会員管理を行うことが出来ます。主な機能は会員登録、ログイン、ログアウトです。ここでは会員登録機能についてのみ解説します。ログインとログアウト機能については、「課題」として考えてみましょう。
	</li>
	<li>アプリ機能について<br/>
...
<ul>
  <li><b>機能説明</b><br/>
	...
	ただし、一つのLineRenderは一本の線しか描けません。また、一つのGameobjectは、一つのLineRendererだけに対応します。ですから、複数の線を描くためには、複数のGameobjectを生成しなければいけません。<br/>
...
<ul>
  <li><b>機能解説</b><br/>
  	<p>この機能を開発するには、mBaaSのファイルストア機能を利用します。画像やテキスト、音楽などさまざまな種類のファイルを保存することができるストレージ機能です。<br/>ここではファイルをクラウドに保存する機能を詳しく説明します。保存したファイルを取得する機能については「課題」として考えてみましょう。<br/>
  ...
  <li><b>ティップ</b>
  <br/>- ログインと同様に、ファイルの保存と取得も同時処理ではありません非同期処理です。
...
<ul>
  <li><b>機能解説</b><br/>
  	...
  	※詳しくは<a href="http://mb.cloud.nifty.com/doc/current/datastore/basic_usage_unity.html">ドキュメント</a>を参照下さい。
  	</p>
  </li>
  <li><b>主な手順
  </b><br/>
...
シーン「doodles」（データを取得する場合）
  ...
  <h5>データを取得する場合</h5>
  	<ul>
	  	...
	  	<li>データを取得する時、先ずは検索条件を決めます。SQL同じ、mBaaSのSDKは、「NCMBQuery.WhereEqualTo(KEY,VALUE)」、「limit」、「skip」などの関数を使って、検索条件を指定することができます。詳しくは<a href="http://mb.cloud.nifty.com/doc/current/datastore/basic_usage_unity.html#基本的な検索の利用">ドキュメント</a>をご参照ください。</li>
  ...
  <li><b>機能解説</b><br/>
  	<ul>
		...
		ここで使用されるmBaaSの機能はデータストアです。データストアの説明は<a href="#q4">機能四</a>の「機能解説」部分をご覧ください。
	 	 ...
	 	 <li>アプリ機能について<br/>
			<ul>
				<li>「Doodle」のランキング機能は、Twitterの「いいね」と似ていて、作品を気に入った人の数で決めます。Doodleの右上のハートマークをクリックすると、ハートマークに色つけ、「お気に入り登録」されます。
				<img height="200px" src="readme-img/f5-01.png"/></li>
				<li>色ついたハートをもう一度クリックすると、以上のアクションを消すことができます。</li>
				<li>
...
シーン「doodles」
	...
	public Sprite likeClickedSprite;
  	...
  	&nbsp;&nbsp;&nbsp;&nbsp;LikeRecordでは、「お気に入り登録」というアクションの記録が保存されてしています。<br/>
	...
	<li>ハートマークが色ついたら、「このDoodleをお気に入り登録した」というアクションをデータストアに保存します。そのため、クラス「LikeRecord」で新しいレコードを生成します：</p>
	<pre>
	...
	<li>色つきハートマークをもう一度クリックしたら、ハートマークは白抜きに戻ります。それに対応して、LikeRecordを消す必要もあります：
	<pre>
  ...
  <li><b>課題</b><br/>
  主な手順で、「お気に入り」機能を作成できましたが、もうあと一歩で、ランキング機能を完成できます。クラス「DoodleRecord」の列「Likes」（Doodleに気にりった人数）を使って、最後の実装をしましょう。難易度★★☆☆☆<br/>
  <答えは<a href="#a5">こちら</a>>
  </li>
</ul>
<span><a href="#keyquestion">リストに戻る</a></span>
...
4. 検索結果とその下のコードが探したい答えですよ！(つ´ω`)つ<br/>
